...
// ------------INPUT DOM ------------------------------------
    ...
    let Least_count=createSpan('Least count : 0.01 mm');
    let Shape_Span=createSpan('Shape : ');
    Shape = createSelect();
...
// ---------------------RANDOM DIM ----------------------
    Random_Dim=createButton('Random Dimention');
...
// ----------------------------SHOW DIM BUTTON -------------------------
    Show_Dim=createButton('Show Dimention');
...
ResponsiveSmallsize=function ResponsiveSmallsize(){
    ...
    // ---------FULL SCREEN BUTTON-----------------------
    fullbtn.position(width/2+240,5*height/6+10).style('font-size','12px').style('background-color','RGB(225,225,225)').style('padding','3px').style('border-radius','2px');
    
    //--------------Zoom IN and OUT--------------------------------
    Zoom_IN.position(width/2+80,5*height/6+10).style('font-size','12px').style('background-color','lightblue').style('padding','3px').style('border-radius','2px');
    Zoom_OUT.position(width/2+140,5*height/6+10).style('font-size','12px').style('background-color','lightgreen').style('padding','3px').style('border-radius','2px');
    ...
    Hide_btn.position(X_I+10,Y_I+5).style('background-color','lightcoral').style('border-radius','5px');
    Shape.position(X_I+120, Y_I+65).size(60).style('font-size','10px');
    
    // OUTPUT
    Dim_Value.position(width/2-200,5*height/6).style('background-color','black').style('font-size','20px').style('color','white').style('padding','15px').style('border-radius','10px');
    Random_Dim.position(width/2-80,5*height/6).style('background-color','green').style('border-radius','50%').size(50,50).style('font-size','8px').style('color','white');
    Show_Dim.position(width/2,5*height/6).style('background-color','blue').size(50,50).style('border-radius','50%').style('font-size','8px').style('color','white');
    }
}