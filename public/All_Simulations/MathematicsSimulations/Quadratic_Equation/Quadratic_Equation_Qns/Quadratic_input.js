function input(){
    ...
    //-------------------QUADRATIC EQUATIONS------------------------------
      ...
      let QUAD=createDiv('QUADRATIC EQUATION');
      // Make Quadratic equation (Y=aX2+bX+C)
      //y
      let y_Span=createSpan('Y=');
      // Make input a,b,c
       //aX2
       A=createElement('INPUT');
       A.attribute('type','number');
       A.value(-1);
      let X_A=createSpan('X');
      let x_square=createSpan('2');
      //bX
     B=createElement('INPUT');
     B.attribute('type','number');
     B.value(2);
      let X_B=createSpan('X');
      //c
    ...
    // -------------------SOLUTIONS----------------------------------
      //output
      let Solution_Box=createDiv('');
      let Show_soln_checkbox = createCheckbox('', false);
      Show_soln_checkbox.changed(Instantoutput);
              function Instantoutput(){
              Show_Sol1=this.checked();
              }
     // SOLUTION
     let SOLUTION=createDiv('SOLUTIONS');
      // Equation Roots  
    ...
    //-------------------VALUE AT Point------------------------------
      let Value_XBox=createDiv('');
      let At_pointX_checkbox = createCheckbox('', false);
      At_pointX_checkbox.changed(showatpointx);
          function showatpointx(){
              Show_Sol2=this.checked();
          }
      // At Point X
      let At_point=createDiv('At Point X');
      //X INPUTS
      let X=createSpan('X :');
      //X VALUE
    ...
    //-------------------DOMAIN AND RANGE------------------------------
      let DOMAIN_Box=createDiv('');
      let Domain_checkbox = createCheckbox('', false);
      ...
      // Domain & Range
      let Domain_Div=createDiv('Domain & Range');
      //DOMAIN INPUTS
      let DOMAIN=createSpan('Domain :  ');
    ...
    Hide_Out_sln.mousePressed(hide_Out_sln);
        ...
        Hide_Out_X.mousePressed(hide_Out_X);
          ...
          let Hide_Out_Domain=createButton('-');
      ...
      if(window.outerWidth<=Small_size && window.outerHeight>window.outerWidth){
        let Dom_elements = [Canvas,fullbtn,Zoom_IN,Zoom_OUT,Input,QUAD,y_Span,X_A,A,B,C,x_square,X_B,Hide_INbtn,Solution_Box,Show_soln_checkbox,SOLUTION,Output_ROOT,Output_Vertex,Output_Yintersect,Output_Axis_OS,Value_XBox,At_pointX_checkbox,At_point,X,X_cord,Y_at_X,Tangent,DOMAIN_Box,Domain_checkbox,Domain_Div,DOMAIN,Domain_type,Domain_X1_Value,Domain_X2_Value,Domain_Range,Hide_Out_sln,Hide_Out_X,Hide_Out_Domain]
        ...
        full_landscape.mousePressed(fulllandscape);
              function fulllandscape(){
                ...
                full_landscape.size(80,20).position(3*height/4+40,9*width/10).style('font-size','10px').style('background-color','RGB(225,225,225)').style('padding','1px').style('border-radius','2px');
    ...
    ResponsiveBigsize=function ResponsiveBigsize(){
        ...
        y_Span.position(Quad_X-50, Quad_Y).style('font-size','30px');
        A.position(Quad_X,Quad_Y).size(40,30).style('font-size','18px');
        X_A.position(Quad_X+60,Quad_Y).style('font-size','30px');
        x_square.position(Quad_X+85,Quad_Y-10).style('font-size','18px');
        B.position(Quad_X+100,Quad_Y).size(40,30).style('font-size','18px');
        X_B.position(Quad_X+160,Quad_Y).style('font-size','30px');
        C.position(Quad_X+200,Quad_Y).size(40,30).style('font-size','18px');
      ...
      //-------------------DOMAIN AND RANGE------------------------------
        ...
        // ------------------ OUTPUT HIDE AND SHOW BUTTON -----------------------------------------------------------------------
        Hide_Out_sln.position(X_O+10,Y_O+5).style('background-color','lightcoral').style('border-radius','5px').style('border','2px solid black');
        Hide_Out_X.position(V_X+10,V_Y+5).style('background-color','lightcoral').style('border-radius','5px').style('border','2px solid black');;
        Hide_Out_Domain.position(D_X+10,D_Y+5).style('background-color','lightcoral').style('border-radius','5px').style('border','2px solid black');;
    ...
    ResponsiveMediumsize=function ResponsiveMediumsize(){
        ...
        B.position(Quad_X+70,Quad_Y).size(25,20);
        X_B.position(Quad_X+110,Quad_Y).style('font-size','20px');
        C.position(Quad_X+130,Quad_Y).size(25,20);
      ...
      //-------------------VALUE AT Point------------------------------
        ...
        // ------------------ OUTPUT HIDE AND SHOW BUTTON -----------------------------------------------------------------------
        Hide_Out_sln.position(X_O+10,Y_O+5).style('background-color','lightcoral').style('border-radius','5px');
        Hide_Out_X.position(V_X+10,V_Y+5).style('background-color','lightcoral').style('border-radius','5px');
        Hide_Out_Domain.position(D_X+10,D_Y+5).style('background-color','lightcoral').style('border-radius','5px');
    ...
    ResponsiveSmallsize=function ResponsiveSmallsize(){
        ...
        X_A.position(Quad_X+35,Quad_Y).style('font-size','15px');
        x_square.position(Quad_X+55,Quad_Y-10).style('font-size','15px');
        B.position(Quad_X+70,Quad_Y).size(20,15).style('font-size','13px');
        X_B.position(Quad_X+110,Quad_Y).style('font-size','15px');
        C.position(Quad_X+130,Quad_Y).size(20,15).style('font-size','13px');
  
        // ----------INPUT HIDE AND SHOW BUTTON--------------------------------------------------------------------------
        Hide_INbtn.position(X_I+10,Y_I+5).style('background-color','lightcoral').style('border-radius','5px').style('border','1px solid black');
  
        // ----------------------------- SOLUTIONS ------------------------------------------------  
        Solution_Box.position(X_O,Y_O).style('padding','5px 110px 140px 110px').style('border-radius','10px').style('border','1px solid black').style('background-color','RGB(225,225,225)');
        Show_soln_checkbox.position(X_O+170,Y_O+10).style('transform','scale(1.3)');
        SOLUTION.position(X_O+70,Y_O+10).style('font-size','15px').style('padding','2px').style('background-color','RGB(160,160,160)');
        Output_ROOT.position(X_O+70,Y_O+35).style('font-size','15px');
        Output_Vertex.position(X_O+65,Y_O+60).style('font-size','15px');
        Output_Yintersect.position(X_O+60,Y_O+85).style('font-size','15px');
        Output_Axis_OS.position(X_O+40,Y_O+110).style('font-size','15px');
  
        //-------------------VALUE AT Point------------------------------
        Value_XBox.position(V_X,V_Y).style('border','1px solid black').style('padding','5px 110px 130px 110px').style('border-radius','10px').style('background-color','RGB(225,225,225)');
        At_pointX_checkbox.position(V_X+170,V_Y+10).style('transform','scale(1.3)');
        At_point.position(V_X+80,V_Y+10).style('font-size','15px').style('padding','2px').style('background-color','RGB(160,160,160)');
        X.position(V_X+80,V_Y+40).style('font-size','15px');
        X_cord.position(V_X+120,V_Y+40).size(20,15);
        Y_at_X.position(V_X+50,V_Y+65).style('font-size','15px');
        Tangent.position(V_X+35,V_Y+90).style('font-size','15px');
  
        //-------------------DOMAIN AND RANGE------------------------------
        DOMAIN_Box.position(D_X,D_Y).style('border-radius','10px').style('border','1px solid black').style('padding','5px 110px 110px 110px').style('background-color','RGB(225,225,225)');
        Domain_checkbox.position(D_X+180,D_Y+10).style('transform','scale(1.3)');
        Domain_Div.position(D_X+50,D_Y+10).style('font-size','15px').style('padding','2px').style('background-color','RGB(160,160,160)');
        DOMAIN.position(D_X+15,D_Y+50).style('font-size','15px');
        Domain_type.position(D_X+70,D_Y+50).style('font-size','14px');
        Domain_X1_Value.position(D_X+155,D_Y+50).size(20,15);
        Domain_X2_Value.position(D_X+186,D_Y+50).size(20,15);
        Domain_Range.position(D_X+45,D_Y+80).style('font-size','15px');
        
        // ------------------ OUTPUT HIDE AND SHOW BUTTON -----------------------------------------------------------------------
        Hide_Out_sln.position(X_O+10,Y_O+5).style('background-color','lightcoral').style('border-radius','5px').style('border','1px solid black');
        Hide_Out_X.position(V_X+10,V_Y+5).style('background-color','lightcoral').style('border-radius','5px').style('border','1px solid black');
        Hide_Out_Domain.position(D_X+10,D_Y+5).style('background-color','lightcoral').style('border-radius','5px').style('border','1px solid black');
